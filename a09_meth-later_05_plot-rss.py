...
#-----------------variable-----------------
fd_rss='./out/a09_meth-later_03_rss'
fd_out='./out/a09_meth-later_05_plot-rss'
...
df=pd.read_csv(f'{fd_rss}/rss_merged.csv', index_col=0)
...
def plot_rss(df, cell, f_out, cmap=cmap, x='rank', fs=(10,8), n=n, hue='hue', title=None):
	...
	plt.xlabel('')
	plt.ylabel('Regulon Specificity Score', fontsize=25, labelpad=20, weight='medium')
	#3.text
	l_x=df[x].tolist()[0:n]
